...
CONFIG_FILE = "ClasificacioText\\Exercici 0\\model_config.json"
...
def main():
    ...
    df = df[[column_idioma, column_text]]
    df = df.dropna()

    labels = df[column_idioma].unique().tolist()
    print("Labels:", labels)
    ...
    le = LabelEncoder()
    df[column_idioma] = le.fit_transform(df[column_idioma].astype(str))
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        df[column_text], df[column_idioma], test_size=0.2, random_state=42
    )
